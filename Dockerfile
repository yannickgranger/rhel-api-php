...
RUN dnf install -y httpd php-fpm

# Configure PHP-FPM
RUN sed -i 's/;listen = 127.0.0.1:9000/listen = \/run\/php-fpm\/www.sock/g' /etc/php-fpm.d/www.conf
RUN sed -i 's/;user = apache/user = apache/g' /etc/php-fpm.d/www.conf
RUN sed -i 's/;group = apache/group = apache/g' /etc/php-fpm.d/www.conf

# Configure Apache to use PHP-FPM (mod_proxy_fcgi)
# Ensure mod_proxy and mod_proxy_fcgi are enabled
RUN sed -i 's/^#LoadModule\s*proxy_module/LoadModule proxy_module/' /etc/httpd/conf.modules.d/00-proxy.conf
RUN sed -i 's/^#LoadModule\s*proxy_fcgi_module/LoadModule proxy_fcgi_module/' /etc/httpd/conf.modules.d/00-proxy.conf
...
CMD ["/bin/bash", "-c", "/usr/sbin/php-fpm -F & /usr/sbin/httpd -D FOREGROUND"]